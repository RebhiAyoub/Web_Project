...
class controller_user_admin
{
    ...
    public function getPaginatedUsers($currentPage, $perPage = 20)
    {
        $sql = "SELECT * FROM user_admin";
        $db = config::getConnexion();
    ...
    public function forget_pwd($email,$new_pwd)
    {
        $sql = "UPDATE user_admin SET password=:new_pwd WHERE email = :email";
        $db = config::getConnexion();
        try {
            $query = $db->prepare($sql);
